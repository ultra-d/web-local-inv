<template>
  <div class="min-h-screen bg-gray-50">
    <AppLayout title="Editar Repuesto">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <h1 class="text-3xl font-bold text-gray-900">Editar Repuesto</h1>
              <p class="mt-2 text-gray-600">Modifica la información del repuesto "{{ part.name }}"</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <form @submit.prevent="submitForm" class="space-y-8 p-8">
            ...
            <section>
              ...
              <div class="space-y-4">
                ...
                <div class="flex items-center justify-center w-full">
                  ...
                  <!-- Preview de la imagen existente o nueva -->
                  <div v-else class="relative w-full">
                    ...
                    <div class="absolute top-2 right-2 flex space-x-2">
                      ...
                      <label
                        for="image-upload"
                        class="bg-blue-500 text-white rounded-full p-2 hover:bg-blue-600 transition-colors cursor-pointer"
            ...
            <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
              <button
                type="button"
                @click="router.get(route('parts.show', part.id))"
              ...
              >
                <span v-if="submitting">Actualizando...</span>
                <span v-else>Actualizar Repuesto</span>
              </button>
            </div>
          </form>
        </div>
...
const currentImageUrl = ref(props.part.image_url || null)
...
const submitForm = async () => {
  try {
    ...
    if (!form.value.codes.length || !form.value.codes.some(code => code.code.trim())) {
    ...
    const validCodes = form.value.codes.filter(code => code.code.trim())
    ...
    validCodes.forEach((code, index) => {
      ...
      formData.append(`codes[${index}][type]`, code.type)
      formData.append(`codes[${index}][is_primary]`, code.is_primary ? '1' : '0')
      formData.append(`codes[${index}][is_active]`, code.is_active ? '1' : '0')
    })
    ...
    // Agregar flag de remover imagen
    ...
    // Enviar con fetch
    const response = await fetch(route('parts.update', props.part.id), {
      method: 'POST',
      body: formData,
      headers: {
    ...
    if (response.ok && responseData.success) {
      // Redirigir al show
      window.location.href = route('parts.show', props.part.id)
    } else {
      if (responseData.errors) {
        errors.value = responseData.errors
      } else {
        errors.value = { general: responseData.message || 'Error al actualizar el repuesto' }
      }
    }
...
const handleModelCreated = (newModel) => {
  ...
  form.value.model_id = newModel.id
